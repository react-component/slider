...
describe('Range', () => {
  ...
  it('should keep pushable with pushable s defalutValue when not allowCross and setState', () => {
    ...
    wrapper.find('.rc-slider').simulate('mouseDown', { button: 0, clientX: 0, clientY: 0 });
    map.mousemove({ type: 'mousemove', clientX: 30, clientY: 0 });
    map.mouseup({ type: 'mouseup', clientX: 30, clientY: 0 });

    expect(wrapper.instance().getSlider().state.bounds[0]).toBe(30);
    expect(wrapper.instance().getSlider().state.bounds[1]).toBe(40);

    wrapper.find('.rc-slider').simulate('mouseDown', { button: 0, clientX: 0, clientY: 0 });
    map.mousemove({ type: 'mousemove', clientX: 50, clientY: 0 });
    map.mouseup({ type: 'mouseup', clientX: 50, clientY: 0 });
